...
import { api, useFetch, useMutate } from "../hooks/useFetch";
...
export function GerenciamentoMesas() {
    ...
    function renderSelectOptions(mesa: Mesa) {
        ...
        for(var i = 1;i <= mesa.qtdMesas;i++)
            rows.push(i);
        return rows.map((i) => <option className="PaginaMesasSelectOptions" selected={mesa.qtdMesas === i} value={i}>{i}</option>);
    }
